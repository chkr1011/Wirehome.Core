...
public sealed class HistoryRepository : IDisposable
{
                    ...
                    using (var valueStream = new HistoryValuesStream(fileStream))
                    {
                        ...
                        while (await valueStream.MoveNextAsync().ConfigureAwait(false))
                        {
                            if (valueStream.CurrentToken is BeginToken beginToken)
                            {
                                ...
                                {
                                    Begin = new DateTime(dayPath.Year, dayPath.Month, dayPath.Day, beginToken.Value.Hours, beginToken.Value.Minutes, beginToken.Value.Seconds,
                                        beginToken.Value.Milliseconds, DateTimeKind.Utc)
                                };
                            }
                            ...
                            {
                                currentElement.End = new DateTime(dayPath.Year, dayPath.Month, dayPath.Day, endToken.Value.Hours, endToken.Value.Minutes, endToken.Value.Seconds,
                                    endToken.Value.Milliseconds, DateTimeKind.Utc);
    ...
    {
        return Path.Combine(date.Year.ToString("0000"), date.Month.ToString("00"), date.Day.ToString("00"));
    }
