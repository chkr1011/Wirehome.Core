...
{
    readonly byte[] _beginTokenIdBuffer =
    {
        (byte)'b'
    };

    readonly byte[] _endTokenIdBuffer =
    {
        (byte)'e'
    };
    ...
    readonly byte[] _valueTokenIdBuffer =
    {
        (byte)'v'
    };
    ...
    {
        var buffer = timeSpan.Hours.ToString("00") + timeSpan.Minutes.ToString("00") + timeSpan.Seconds.ToString("00") + timeSpan.Milliseconds.ToString("000");
    ...
    TimeSpan ParseTimeSpan(ReadOnlySpan<byte> source)
    {
        ...
        return new TimeSpan(0, int.Parse(buffer.Slice(0, 2), NumberStyles.None), int.Parse(buffer.Slice(2, 2), NumberStyles.None), int.Parse(buffer.Slice(4, 2), NumberStyles.None),
