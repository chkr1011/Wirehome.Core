...
public sealed class ComponentGroupRegistryService : WirehomeCoreService
{
    ...
    public ComponentGroupRegistryService(StorageService storageService,
        ...
        ILogger<ComponentGroupRegistryService> logger)
    {
        ...
        appService.RegisterStatusProvider("componentGroups", () => { return GetComponentGroups().Select(ComponentGroupsController.CreateComponentGroupModel); });
    }
    ...
    public void InitializeComponentGroup(string uid)
    {
        ...
        {
            if (!_storageService.TryReadSerializedValue(out IDictionary<string, object> associationSettings, ComponentGroupsDirectory, uid, "Components", associationUid,
        ...
        foreach (var associationUid in associationUids)
        {
            if (!_storageService.TryReadSerializedValue(out IDictionary<string, object> associationSettings, ComponentGroupsDirectory, uid, "Macros", associationUid,
                    DefaultFileNames.Settings))
            {
                associationSettings = new Dictionary<string, object>();
            }
    ...
    public object RemoveComponentGroupSetting(string componentGroupUid, string settingUid)
    {
        ...
        _logger.Log(LogLevel.Debug, "Component group '{0}' setting '{1}' removed.", componentGroupUid, settingUid);
    ...
    public void SetComponentGroupSetting(string componentGroupUid, string settingUid, object value)
    {
        ...
        _logger.Log(LogLevel.Debug, "Component group '{0}' setting '{1}' changed from '{2}' to '{3}'.", componentGroupUid, settingUid, setSettingResult.OldValue ?? "<null>",
            value ?? "<null>");
